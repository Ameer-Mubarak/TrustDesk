    ...
    <Modal title="Create remediation task" onClose={onClose}>
      <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); onSubmit(form); }}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label><span className="label">Vendor</span><select className="field mt-1" value={form.vendor_id ?? ''} onChange={(e) => setForm({ ...form, vendor_id: e.target.value || null })}><option value="">No vendor</option>{vendors.map((vendor) => <option value={vendor.id} key={vendor.id}>{vendor.name}</option>)}</select></label>
